/**
 * Sistema de Autenticación - Bombas Bloch
 * Simulado con localStorage para demo
 */

class AuthSystem {
    constructor() {
        this.currentUser = JSON.parse(localStorage.getItem('bb_user')) || null;
        this.users = JSON.parse(localStorage.getItem('bb_users')) || [];
        this.orders = JSON.parse(localStorage.getItem('bb_orders')) || [];
        this.init();
    }
    ...
    init() {
        ...
        if (!this.users.find(u => u.email === 'admin@bombasbloch.com')) {
            this.users.push({
                id: 'admin',
